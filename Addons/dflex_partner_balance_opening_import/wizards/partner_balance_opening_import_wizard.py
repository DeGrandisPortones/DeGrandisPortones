\
    ...
    class DflexPartnerBalanceOpeningImportWizard(models.TransientModel):
        ...
        def _prepare_move_vals(self, row, partner, amount):
            ...
            if amount >= 0:
                ...
                capital_debit, capital_credit = Decimal("0.00"), abs_amount
            else:
                partner_debit, partner_credit = Decimal("0.00"), abs_amount
                capital_debit, capital_credit = abs_amount, Decimal("0.00")
            ...
            return {
                ...
                "line_ids": [
                    ...
                    (0, 0, {
                        ...
                        "debit": float(capital_debit),
                        "credit": float(capital_credit),
                    }),
                ],
            }
        ...
        def action_import(self):
            ...
            for row in rows:
                ...
                if not wizard.dry_run:
                    if not partner:
                        continue
                    move = Move.create(wizard._prepare_move_vals(row, partner, amount))
            ...
            post_status = "publicados" if wizard.post_moves else "en borrador"
            output = [
                mode,
                ...
                "Asientos: %s" % post_status,
            ]
            if not wizard.dry_run:
                output.append("Asientos creados: %s" % len(created_moves))
            if errors:
                output.append("")
            ...
            # In real mode, block partial imports when there are missing partners and creation was not allowed.
