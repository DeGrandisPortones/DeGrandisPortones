...
class PresupuestadorRubro(models.Model):
    ...
    # Simple allow-listing for products
    product_category_id = fields.Many2one("product.category", string="Categoría (opcional)")
    product_tag_ids = fields.Many2many("product.tag", string="Etiquetas (opcional)")
    active = fields.Boolean(default=True)


class PresupuestadorPedido(models.Model):
    _name = "presupuestador.pedido"
    _description = "Pedido de Presupuesto (SPA/Portal)"
...
class PresupuestadorLinea(models.Model):
    ...
    precio_distr = fields.Monetary(string="Precio base", currency_field="currency_id", readonly=True)
    price_unit = fields.Monetary(string="Precio final", currency_field="currency_id", readonly=True)
