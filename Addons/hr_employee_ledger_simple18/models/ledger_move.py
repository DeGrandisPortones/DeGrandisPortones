...
class EmployeeLedgerMove(models.Model):
    ...
    type = fields.Selection([
        ...
        ("descuento", "Descuento"),
        ("pago", "Pago"),
    ], string="Tipo", required=True, default="general")
    direction = fields.Selection([
        ("in", "Entrada"),
        ("out", "Salida"),
    ], string="Dirección", required=True, default="in")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("name") or vals.get("name") == "New":
                seq = self.env["ir.sequence"].next_by_code("hr.employee.ledger.move") or "New"
                vals["name"] = seq
