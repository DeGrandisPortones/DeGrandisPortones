...
class SaleOrder(models.Model):
    ...
    # % global (puede venir como 0..100 o como fracción 0..1 si usás widget percentage)
    global_discount_rate = fields.Float(
        ...
        help="Porcentaje aplicado a todas las líneas antes del cálculo de impuestos. "
             "Con widget de porcentaje puede guardarse como fracción (0..1).",
        default=0.0,
        digits=(16, 4),
    )

    # Importe descontado sobre la base (antes de impuestos)
    ...
    # Subtotal con descuento de línea pero ANTES del descuento global
    ...
    # NUEVO: Total con descuento (espejo del total ya recalculado)
    amount_total_after_global = fields.Monetary(
        string="Total con descuento",
        compute="_amount_all",
        store=True,
        currency_field="currency_id",
        help="Total final con descuento global aplicado en la base e impuestos recalculados.",
    )
    ...
    def _amount_all(self):
        """
        Aplica un descuento global adicional por línea ANTES de impuestos,
        combinándolo con el descuento propio de la línea (multiplicativo).
        Recalcula amount_untaxed, amount_tax y amount_total sobre la base ya descontada.
        """
        for order in self:
            ...
            amount_untaxed = 0.0                 # Subtotal con descuento global
            amount_tax = 0.0
            discount_base_total = 0.0            # Importe descontado por el global
            subtotal_before_global = 0.0         # Subtotal ANTES del global
            groups = defaultdict(lambda: {'base': 0.0, 'amount': 0.0, 'group': False})

            # Normalizar global: 0..1 => fracción; 0..100 => dividir por 100
            ...
            for line in order.order_line.filtered(lambda l: not l.display_type):
                ...
                # Subtotal de la línea con SOLO descuento de línea
                ...
                # Importe del DESCUENTO GLOBAL puro en esta línea
                ...
                # (Opcional) Acumular por grupo de impuestos si tu edición lo usa
            ...
            order.amount_total_after_global = order.amount_total  # espejo, para mostrar fila propia

            # Solo setear amount_by_group si existe en esta base
