...
class SaleOrder(models.Model):
    ...
    def _check_global_discount_rate(self):
        for order in self:
            if order.global_discount_rate < 0.0 or order.global_discount_rate > 100.0:
                raise ValidationError("El Descuento global (%) debe estar entre 0 y 100.")
    ...
    def _amount_all(self):
        ...
        for order in self:
            ...
            # ⚠️ Solo setear amount_by_group si el campo existe en este entorno
            if "amount_by_group" in order._fields:
                order.amount_by_group = self._format_amount_by_group(groups, currency)

    # Esta helper no hace daño aunque no uses amount_by_group
